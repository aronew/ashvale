...
export const SWINGS=[{name:'Crescent cut',duration:.36,impact:.10,range:78,multiplier:1,cone:-.1},{name:'Rising backhand',duration:.34,impact:.09,range:84,multiplier:1.15,cone:-.4},{name:'Sundering cleave',duration:.52,impact:.20,range:98,multiplier:1.75,cone:-.55}];
...
export class Game{
 constructor(saved){
  ...
  this.player={x:25.5*TILE,y:36*TILE,hp:100,maxHp:100,stamina:100,xp:0,level:1,dir:0,angle:-Math.PI/2,attack:0,dash:0,dashCd:0,spell:0,invincible:0,kills:0,combo:0,comboTimer:0,attackDuration:0,attackAngle:0,attackHit:true,attackBuffer:0};
  this.bag={wood:0,stone:0,herb:0,essence:0,potion:3,core:0};this.flags={met:false,cottage:false,boss:false,hearth:false,upgrade:false,beacon0:false,beacon1:false,beacon2:false,beacons:false};this.removed=new Set();this.harvests={};this.lastSave=0;
 ...
 build(){
  ...
  // Append expansion objects so every original save retains its object IDs.
  ...
  [[64,38],[68,40],[71,45],[65,47],[73,39],[69,50]].forEach((p,i)=>this.enemies.push({id:'wisp'+i,type:'wisp',x:p[0]*TILE,y:p[1]*TILE,homeX:p[0]*TILE,homeY:p[1]*TILE,hp:85,maxHp:85,cd:1+i*.2,hit:0,phase:i,windup:0,cast:0}));
 ...
 attack(){
  ...
  const swing=SWINGS[p.combo];p.attack=swing.duration;p.attackDuration=swing.duration;p.attackHit=false;p.attackBuffer=0;p.comboTimer=swing.duration+.7;
  const nearest=this.enemies.filter(e=>e.hp>0&&distance(e,p)<100).sort((a,b)=>distance(a,p)-distance(b,p))[0];
  if(nearest)p.angle=Math.atan2(nearest.y-p.y,nearest.x-p.x);
  p.attackAngle=p.angle;p.dir=Math.abs(Math.cos(p.angle))>Math.abs(Math.sin(p.angle))?Math.cos(p.angle)<0?2:3:Math.sin(p.angle)<0?1:0;
  this.event('attack',{combo:p.combo});return true;
 }
 strike(){
  const p=this.player,swing=SWINGS[p.combo];let hits=0;
  for(const e of this.enemies)if(e.hp>0&&distance(e,p)<swing.range+(e.type==='guardian'?14:0)&&Math.cos(Math.atan2(e.y-p.y,e.x-p.x)-p.attackAngle)>swing.cone){this.hitEnemy(e,Math.round(((this.flags.upgrade?24:17)+(p.level-1)*3+(this.flags.beacons?8:0))*swing.multiplier));hits++;}
  for(const o of this.objects)if(!this.removed.has(o.id)&&['tree','ore','herb'].includes(o.type)&&distance(o,p)<(p.combo===2?78:68)&&Math.cos(Math.atan2(o.y-p.y,o.x-p.x)-p.attackAngle)>-.4){o.hp-=p.combo===2?2:1;o.hit=.18;this.particlesAt(o.x,o.y-15,o.type==='tree'?'#78a887':o.type==='ore'?'#a9b8b6':'#dd92b2',p.combo===2?15:7);this.event('chop');hits++;if(o.hp<=0){this.removed.add(o.id);this.gain(o.type==='tree'?'wood':o.type==='ore'?'stone':'herb',o.type==='tree'?4:o.type==='ore'?3:2,o.x,o.y);this.exp(5);}}
  if(hits)this.event('impact',{combo:p.combo});
  if(p.combo===2)this.particlesAt(p.x+Math.cos(p.attackAngle)*48,p.y+Math.sin(p.attackAngle)*48-12,'#f6cb86',18);
 }
 hitEnemy(e,amount){e.hp-=amount;e.hit=.15;const p=this.player,a=Math.atan2(e.y-p.y,e.x-p.x);this.move(e,Math.cos(a)*(p.combo===2?25:13),Math.sin(a)*(p.combo===2?25:13),8);this.float(String(amount),e.x,e.y-27,'#f4dbc3');this.particlesAt(e.x,e.y-10,'#a986c2',8);if(e.hp<=0){p.kills++;this.exp(e.type==='guardian'?100:e.type==='wisp'?30:18);this.gain('essence',e.type==='guardian'?8:e.type==='wisp'?3:2,e.x,e.y);if(e.type==='guardian'){this.flags.boss=true;this.gain('core',1,e.x,e.y);this.toast('The Heart Spark is yours. Return to the village hearth.');this.event('boss-defeated');}else if(p.kills%3===0)this.gain('herb',1,e.x,e.y);}}
 ...
 interact(){const o=this.nearest();if(!o)return false;if(o.type==='beacon'){this.lightBeacon(o);}else if(o.type==='npc'){this.flags.met=true;this.event('dialogue');}else if(o.type==='chest'){this.removed.add(o.id);this.gain('potion',1,o.x,o.y);this.gain('wood',3,o.x,o.y);this.gain('stone',2,o.x,o.y);this.toast('Supply cache opened.');}else if(o.type==='herb'){this.removed.add(o.id);this.gain('herb',2,o.x,o.y);}else if(o.type==='ruin'){this.event('repair');}else if(o.type==='cottage'){this.player.hp=this.player.maxHp;this.toast('You rest at the Lantern Inn. Health restored.');this.event('heal');}else if(o.type==='hearth'){this.event('hearth');}else if(o.type==='arch'){this.toast('The Rootvault · Follow the stone passage east.');}else if(o.type==='plot'){if(o.ripe){this.gain('herb',2,o.x,o.y);o.ripe=false;o.growing=0;this.toast('Moonleaf harvested. The soil is ready again.');}else if(!o.growing){o.growing=30;this.toast('Moonleaf planted. It will grow in 30 seconds.');}else this.toast('Growing · '+Math.ceil(o.growing)+' seconds until harvest.');}return true;}
 ...
 lightBeacon(o){
  ...
  if([0,1,2].every(i=>this.flags['beacon'+i])){this.flags.beacons=true;this.bag.potion+=3;this.exp(100);this.event('moonfen-complete');}else this.toast('A beacon burns again. Health restored.');return true;
 }
 update(dt,input){this.time+=dt;const p=this.player;for(const k of ['attack','dash','dashCd','spell','invincible','comboTimer','attackBuffer'])p[k]=Math.max(0,p[k]-dt);p.stamina=Math.min(100,p.stamina+dt*(this.flags.beacons?25:19));
  let dx=input.x||0,dy=input.y||0,mag=Math.hypot(dx,dy);if(mag){dx/=mag;dy/=mag;p.angle=Math.atan2(dy,dx);if(p.attack<=0)p.dir=Math.abs(dx)>Math.abs(dy)?dx<0?2:3:dy<0?1:0;}
  ...
  if(!p.attackHit&&p.attackDuration-p.attack>=SWINGS[p.combo].impact){p.attackHit=true;this.strike();}
  if(p.attack<=0&&(input.attack||p.attackBuffer>0))this.attack();for(const o of this.objects){o.hit=Math.max(0,(o.hit||0)-dt);if(o.growing>0){o.growing=Math.max(0,o.growing-dt);if(o.growing===0)o.ripe=true;}}
  for(const e of this.enemies){if(e.hp<=0)continue;e.cd=Math.max(0,e.cd-dt);e.hit=Math.max(0,e.hit-dt);e.phase+=dt;
   if(e.type==='wisp'){
    const d=distance(e,p),a=Math.atan2(p.y-e.y,p.x-e.x);
    if(d<310&&this.flags.hearth){
     if(e.cast>0){e.cast-=dt;if(e.cast<=0){this.projectiles.push({x:e.x,y:e.y,vx:Math.cos(e.aim)*175,vy:Math.sin(e.aim)*175,life:2.5});e.cd=2.1;}}
     else if(e.cd<=0){e.cast=.65;e.aim=a;}
     else if(d>145)this.move(e,Math.cos(a)*dt*48,Math.sin(a)*dt*48);
     else if(d<85)this.move(e,-Math.cos(a)*dt*35,-Math.sin(a)*dt*35);
    }else e.cast=0;
    continue;
   }
   const d=distance(e,p),boss=e.type==='guardian';if(d<(boss?320:235)&&d>29){let a=Math.atan2(p.y-e.y,p.x-e.x),speed=boss?47:e.type==='bat'?88:60;if(e.windup<=0)this.move(e,Math.cos(a)*dt*speed,Math.sin(a)*dt*speed,9);}else if(d>350&&distance(e,{x:e.homeX,y:e.homeY})>10){const a=Math.atan2(e.homeY-e.y,e.homeX-e.x);this.move(e,Math.cos(a)*dt*35,Math.sin(a)*dt*35);}
  ...
  for(const bolt of this.projectiles){bolt.life-=dt;bolt.x+=bolt.vx*dt;bolt.y+=bolt.vy*dt;if(distance(bolt,p)<16){this.hurt(14);bolt.life=0;}if([0,3].includes(this.tile(bolt.x,bolt.y)))bolt.life=0;}this.projectiles=this.projectiles.filter(b=>b.life>0);
